...
class XunleiHijackDownloader(_PluginBase):
    ...
    def _build_page_metrics_poller_onerror(plugin_id: str, interval_ms: int = 1000) -> str:
        ...
        return (
            ...
            "const glyph=(apiText.indexOf('/task/start')>=0)?'▶':'❚❚';"
            "const glyphColor=(apiText.indexOf('/task/start')>=0)?'#4CAF50':'#FB8C00';"
            "if(prepend){prepend.style.display='none';}"
            "if(content){content.style.display='inline-flex';content.style.alignItems='center';content.style.justifyContent='center';content.style.fontSize='13px';content.style.lineHeight='1';content.style.fontWeight='700';content.style.color=glyphColor;content.textContent=glyph;}"
            "node.style.color=glyphColor;"
