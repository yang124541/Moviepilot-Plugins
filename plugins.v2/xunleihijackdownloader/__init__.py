...
class XunleiHijackDownloader(_PluginBase):
    ...
    def _build_page_metrics_poller_onerror(plugin_id: str, interval_ms: int = 5000) -> str:
        ...
        return (
            ...
            "if(!prepend){return;}"
            "let iconEl=prepend.querySelector('.v-icon');"
            "const needRebuild=(!iconEl)||(!!(iconEl&&iconEl.querySelector&&iconEl.querySelector('svg')));"
            "if(needRebuild){"
            "prepend.innerHTML='';"
            "iconEl=document.createElement('i');"
            "iconEl.className='v-icon notranslate mdi '+name;"
            "iconEl.setAttribute('aria-hidden','true');"
            "prepend.appendChild(iconEl);"
            "return;"
            "}"
            "iconEl.classList.remove('mdi-play','mdi-pause');"
            ...
            "forceActionIcon(btn,useStart?'mdi-play':'mdi-pause');"
