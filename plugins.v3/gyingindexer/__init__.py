...
class GyingIndexer(_PluginBase):
    ...
    def _register_builtin_indexer(self) -> None:
        logger.debug("观影(GYing)使用 MoviePilot V3 插件资源源，不再注册 SitesHelper 索引器。")

    def _plugin_site(self) -> Dict[str, Any]:
        """为 V3 插件资源源调用构造内部站点配置。"""
        host = (self._ordered_extra_hosts() or ["xn--kivn76b41nnhi.com"])[0]
        return {
            "id": "gying",
            "name": self.plugin_name,
            "domain": host,
            "url": self._preferred_site_base_url(host),
            "timeout": 20,
            "proxy": True,
        }
